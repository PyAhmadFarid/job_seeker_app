...
<body class="antialiased">
    ...
    <div class="  w-screen h-screen flex">
        <div class=" bg-white w-60 border-r">
            <div class="w-full">
            ...
            <div class=" bg-w">
                ...
                <div
                    class="{{ $prefix == '/jobs' ? 'border-r-4' : '' }} py-2 px-4 my-2 w-full flex items-center space-x-5 border-blue-500 hover:bg-gray-100 transition-all  font-semibold">
                    ...
                    <a href="{{ route('jobs') }}" class=" w-full  text-gray-600">Jobs</a>
                </div>
                <div
                    class="{{ $prefix == '/applicants' ? 'border-r-4' : '' }} py-2 px-4 my-2 w-full flex items-center space-x-5 border-blue-500 hover:bg-gray-100 transition-all  font-semibold">

                    <svg xmlns="http://www.w3.org/2000/svg" width="30" viewBox="0 0 24 24">
                        <path fill="#6b7280"
                            d="M12.3,12.22A4.92,4.92,0,0,0,14,8.5a5,5,0,0,0-10,0,4.92,4.92,0,0,0,1.7,3.72A8,8,0,0,0,1,19.5a1,1,0,0,0,2,0,6,6,0,0,1,12,0,1,1,0,0,0,2,0A8,8,0,0,0,12.3,12.22ZM9,11.5a3,3,0,1,1,3-3A3,3,0,0,1,9,11.5Zm9.74.32A5,5,0,0,0,15,3.5a1,1,0,0,0,0,2,3,3,0,0,1,3,3,3,3,0,0,1-1.5,2.59,1,1,0,0,0-.5.84,1,1,0,0,0,.45.86l.39.26.13.07a7,7,0,0,1,4,6.38,1,1,0,0,0,2,0A9,9,0,0,0,18.74,11.82Z" />
                    </svg>
                    <a href="{{ route('applicants') }}" class="  w-full text-gray-600">Applicants</a>
                </div>

                @if (auth()->user()->role == 0)
                    <div
                        class="{{ $prefix == '/admins' ? 'border-r-4' : '' }} py-2 px-4 my-2 w-full flex items-center space-x-5 border-blue-500 hover:bg-gray-100 transition-all  font-semibold">

                        <svg xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="30" viewBox="0 0 24 24">
                            <path fill="#6b7280"
                                d="M12,2A10,10,0,0,0,4.65,18.76h0a10,10,0,0,0,14.7,0h0A10,10,0,0,0,12,2Zm0,18a8,8,0,0,1-5.55-2.25,6,6,0,0,1,11.1,0A8,8,0,0,1,12,20ZM10,10a2,2,0,1,1,2,2A2,2,0,0,1,10,10Zm8.91,6A8,8,0,0,0,15,12.62a4,4,0,1,0-6,0A8,8,0,0,0,5.09,16,7.92,7.92,0,0,1,4,12a8,8,0,0,1,16,0A7.92,7.92,0,0,1,18.91,16Z" />
                        </svg>
                        <a href="{{ route('admins') }}" class="  w-full text-gray-600">Admins</a>
                    </div>
                @endif


                <div
                    class="{{ $prefix == '/export' ? 'border-r-4' : '' }} py-2 px-4 my-2 w-full flex items-center space-x-5 border-blue-500 hover:bg-gray-100 transition-all  font-semibold">
                    ...
                    <a href="{{route('export')}}" class="  w-full text-gray-600">Export Data</a>
                </div>
            </div>
        </div>
        <div class="flex-1 bg-gray-50  flex-col flex">
            <div class="bg-white flex justify-between items-center py-3 px-10">
                ...
                <a href="{{route('home')}}" class="font-bold text-blue-500 text-xl">PT. Fokus Jasa Mitra </a>
                <div class="flex items-center space-x-5 " x-data="{'isModalOpen': false}">
                    ...
                    <img x-on:click="isModalOpen = !isModalOpen"
                        ...
                        src="{{ auth()->user()->profile_picture? Storage::url(auth()->user()->profile_picture): url('/images/default_profil.jpg') }}"
                        alt="Image" />
                    <div class=" fixed p-3 bg-white border rounded-md mt-48 w-52" x-show="isModalOpen" x-transition
                        @click.away="isModalOpen = false">
                        <a href="{{route('admins.edit',['userid'=>auth()->user()->id])}}" class=" bg-gray-200 p-3 w-full mb-4 rounded-md block text-center">Setting</a>
                        <a href="{{ route('logout') }}"
